...
Build the container image with `docker build -t pinball-api:$VERSION .` and publish it to a private registry. Two common options are:
...
2. **Self-hosted registry in Portainer** – Portainer includes a registry service that can run alongside your stacks. Expose it over HTTPS and create a service account with `docker login <portainer-host>:<port>` credentials. GitHub Actions can then push images directly to this registry.

Portainer can watch either registry via webhooks—trigger a webhook after pushing `dev`, `stg` or `prd` tags. Each tag corresponds to its own stack with environment-specific variables for databases, secrets and SMTP credentials. This keeps the code private while enabling automated updates across environments.
