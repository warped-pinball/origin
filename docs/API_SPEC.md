...
- **Self‑contained sign‑up and login** with optional two‑factor verification. Email is used for account recovery.
...
1. **User Accounts**
   ...
   - Two‑factor options: email verification codes or time‑based one‑time passwords (TOTP).
...
1. **Sign‑Up**
   ...
   - Service sends a verification link via email.
   - Upon verification the account becomes active.
   - Optionally allow enabling a TOTP app (e.g., Google Authenticator) for 2FA.

2. **Login**
   - Users authenticate with email address and password (username not required for login).
...
3. **Password and Email Changes**
   ...
   - When changing email address, send a confirmation link to the new address.
...
Use a service such as Brevo to deliver verification and password reset messages via email. Store the API key and sender address in environment variables in `docker-compose.yml` so containers can send messages without rebuilding the image.
