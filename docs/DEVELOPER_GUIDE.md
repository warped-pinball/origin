...
The resulting `sdks/typescript/dist/index.js` file is bundled with the web host when you run `npm --prefix web build`. The build script copies the file to `app/static/js/api.js`. The service worker is a simple static file located in `app/static/js`.
